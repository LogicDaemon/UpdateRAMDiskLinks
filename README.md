...
- **`:env`:** Define dynamic environment variables to use in path resolutions. Supports recursive expansion. Before resolving a `:env` block, the utility temporarily removes any non-optional variables declared by that block from both the cached config environment and the process environment, so dependent values are recomputed from the config instead of stale inherited values. Keys starting with `?` (e.g. `"?APPDATA"`) are left intact during that reset, so pre-existing values keep winning and the config only fills them in when they are truly missing or empty. If a non-optional declared variable still cannot be resolved, the previous value is restored and a warning is logged.
